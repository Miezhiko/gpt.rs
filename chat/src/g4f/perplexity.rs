...
pub struct PerplexityLabsGenerator;

#[async_trait]
impl Generator for PerplexityLabsGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          rspns = g4f.ChatCompletion.create( model="gemma-2-27b-it", messages=messages
                                           , stream=False, auth="jwt"
                                           , provider=g4f.Provider.PerplexityLabs )
          if not rspns:
            result = "PerplexityLabs: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use PerplexityLabs now!") }
    }
  }
}
...
mod perplexity_tests {
  use super::*;
  #[tokio::test]
  async fn perplexity_test() {
    let gen = PerplexityLabsGenerator;
    let chat_response =
