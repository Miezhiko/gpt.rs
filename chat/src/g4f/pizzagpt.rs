...
pub struct PizzagptGenerator;

#[async_trait]
impl Generator for PizzagptGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "Pizzagpt: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use Pizzagpt now!") }
    }
  }
}
