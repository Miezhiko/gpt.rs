...
pub struct FeedoughGenerator;

#[async_trait]
impl Generator for FeedoughGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "Feedough: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use Feedough now!") }
    }
  }
}
...
mod feedough_tests {
  use super::*;
  #[tokio::test]
  async fn feedough_test() {
    let gen = FeedoughGenerator;
    let chat_response =
