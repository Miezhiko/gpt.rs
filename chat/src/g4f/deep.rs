...
#[async_trait]
impl Generator for DeepGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          rspns = g4f.ChatCompletion.create( model="dbrx-instruct", messages=messages
                                           , stream=False, auth="jwt"
                                           , provider=g4f.Provider.DeepInfra )
          if not rspns:
            result = "DeepGenerator: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use DeepGenerator now!") }
    }
  }
}
