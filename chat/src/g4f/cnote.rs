...
pub struct CnoteGenerator;

#[async_trait]
impl Generator for CnoteGenerator {
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          if not rspns:
            result = "Cnote: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use Cnote now!") }
    }
  }
}
