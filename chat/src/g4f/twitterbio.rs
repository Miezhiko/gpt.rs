...
pub struct TwitterBioGenerator;

#[async_trait]
impl Generator for TwitterBioGenerator {
  ...
  async fn call( &self
    ...
    match catch_unwind(|| {
      ...
      c.run(python! {
        ...
        try:
          ...
          rspns = g4f.ChatCompletion.create( model="gpt-3.5-turbo", messages=messages
                                           ...
                                           , provider=g4f.Provider.TwitterBio )
          if not rspns:
            result = "TwitterBio: Sorry, I can't generate a response right now."
            reslt = False
          else:
            result = rspns
            reslt = True
      ...
      }, Err(_) => { bail!("Failed to to use TwitterBio now!") }
    }
  }
}

#[cfg(test)]
mod twitterbio_tests {
  use super::*;
  #[tokio::test]
  async fn twitterbio_test() {
    let gen = TwitterBioGenerator;
    let chat_response =
